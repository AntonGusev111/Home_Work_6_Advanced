...
def remove_doc_from_shelf(doc_number):
    for directory_number, directory_docs_list in directories.items():
        if doc_number in directory_docs_list:
            ...
            return (f'documet {doc_number} deleted ')
            break
...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    return f'document {doc_number} add to {shelf_number}'
...
def show_all_docs_info():
    dock_list = []
    dock_list.append('Список всех документов:')
    for current_document in documents:
        dock_list.append(show_document_info(current_document))
    return dock_list
